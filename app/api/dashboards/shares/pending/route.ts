...
export async function GET(request: Request) {
  try {
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          eq(sharedDashboards.status, 'pending')
        )
      );

    // Enrichir avec owner & dashboard info
    ...
    return NextResponse.json({ ok: true, invites: enriched });
  } catch (error) {
    console.error('Pending shares error:', error);
